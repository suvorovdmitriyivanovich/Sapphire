...
import com.dealerpilothr.models.AccountData;
    ...
    protected Object doInBackground(Object... arg0) {
        ...
        if (responseData.getSuccess()) {
            ...
            if (accountDatas.size() == 1) {
                ...
                userInfo.setMasterOrganization(accountDatas.get(0).getOrganizations().get(0));
                userInfo.setAccountSession(accountSession);
                userInfo.setGlobalAppRoleAppSecurities(accountDatas.get(0).getGlobalAppRoleAppSecurities());
                ...
                if (navigationMenuDatas != null && navigationMenuDatas.size() > 0) {
                    DBHelper.getInstance(Dealerpilothr.getInstance()).deleteNavigationMenus();
                    DBHelper.getInstance(Dealerpilothr.getInstance()).addNavigationMenus(navigationMenuDatas, accountDatas.get(0).getGlobalAppRoleAppSecurities());
                }

                //userInfo.getProfile().setAvatarId("6993dd39-d2da-8230-6ad8-86f3839db097");

                String folder = Dealerpilothr.getInstance().getFilesDir().getAbsolutePath();
